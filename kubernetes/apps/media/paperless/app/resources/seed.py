...
TAGS = [
    # inbox tag — applied to ALL new documents automatically, no match rule needed
    {"name": "inbox",        "match": "",                                                                "matching_algorithm": 0, "is_inbox_tag": True},
    # manual-only tags — no auto-match
    {"name": "needs-action", "match": "",                                                                "matching_algorithm": 0, "is_inbox_tag": False},
    {"name": "personal",     "match": "",                                                                "matching_algorithm": 0, "is_inbox_tag": False},
    {"name": "work",         "match": "",                                                                "matching_algorithm": 0, "is_inbox_tag": False},
    # content-matched tags
    {"name": "financial",    "match": "invoice receipt statement payment amount due balance owing",      "matching_algorithm": 1, "is_inbox_tag": False},
    {"name": "tax",          "match": "T4 T1 CRA notice of assessment tax return RRSP",                 "matching_algorithm": 1, "is_inbox_tag": False},
    {"name": "medical",      "match": "prescription patient physician clinic hospital",                  "matching_algorithm": 1, "is_inbox_tag": False},
    {"name": "government",   "match": "Government of Canada Province of Ontario Ministry",               "matching_algorithm": 1, "is_inbox_tag": False},
    {"name": "contract",     "match": "agreement contract terms and conditions",                         "matching_algorithm": 1, "is_inbox_tag": False},
    {"name": "receipt",      "match": "receipt thank you for your purchase",                            "matching_algorithm": 1, "is_inbox_tag": False},
    {"name": "insurance",    "match": "policy number premium insured coverage deductible",               "matching_algorithm": 1, "is_inbox_tag": False},
    {"name": "utilities",    "match": "hydro electricity natural gas utility",                           "matching_algorithm": 1, "is_inbox_tag": False},
]
...
for dt in DOCUMENT_TYPES:
    ...
    print(f"  {'created' if created else 'updated '} → {dt['name']}")
...
for t in TAGS:
    ...
    print(f"  {'created' if created else 'updated '} → {t['name']}")

print("Seeding correspondents...")
for c in CORRESPONDENTS:
    obj, created = Correspondent.objects.update_or_create(
        name=c["name"],
