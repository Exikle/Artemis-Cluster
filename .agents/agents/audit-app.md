...
| `postgres/cert` | **The app's driver can present a PEM client cert.** `pooler-rw` is `auth_type: cert`; there is no password (`postgres-dragonfly.md`).                                                                                                                                                                                                                                                                                           | Step 2.                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
4. **Check the connection budget before recommending any onboarding.** `max_connections` on the
   shared cluster and the sum of pooler and sidecar pool sizes are a hard ceiling that no single
