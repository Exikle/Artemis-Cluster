...
- **YAML anchor ordering**: YAML requires an anchor (`&foo`) to appear in the document before any alias (`*foo`) that references it. When alphabetical sorting would place an anchor definition after its first alias use, keep the anchor definition at the top of its section — treat it like `enabled` (always first). This takes precedence over the default alphabetical rule.
