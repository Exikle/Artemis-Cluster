...
| -------------------------------------- | ----------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `../../../../components/kopiur/backup` | kopiur `Snapshot` + `SnapshotPolicy` + the app PVC          | `APP`, `KOPIUR_CAPACITY` (defaults `5Gi`); `KOPIUR_PUID`/`KOPIUR_PGID` (both default `1000`); `KOPIUR_STORAGECLASS` and `KOPIUR_CACHE_STORAGECLASS` (both default `ceph-block`, separate on purpose — the mover cache must not follow the app PVC onto a replicated class) |
