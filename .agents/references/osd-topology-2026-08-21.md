# Reference: OSD topology and drive selection — evaluation 2026-08-21

A dated decision record, not a live reference. It is the written recommendation issue #1524 §5
asked for: measured drive health, endurance, failure-domain math, where a 4th and 5th OSD could
physically go, and costed options A/B/C ending in a recommended order of work. **Nothing here was
applied** — no OSD was added and the CRUSH map was not touched. Keep it because the rules that
_are_ live in `rook-ceph.md` (3 OSDs and do not add, never two OSDs on `pantheon` VMs, the
model-locked `devicePathFilter`, `pg_num` stays 64) are only justified by the measurements below.
Re-measure before acting on the pricing or the wear figures — they were current on 2026-08-21 and
nothing refreshes them. Operational Ceph procedure lives in `rook-ceph.md`; the drive swaps
themselves are the `osd-rebuild` skill.
